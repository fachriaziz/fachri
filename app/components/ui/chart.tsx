        ...
        <div className="grid gap-1.5">
          {payload.map((item, index) => {
            ...
            const indicatorColor =
              color ||
              (item as { payload?: { fill?: string }; color?: string }).payload
                ?.fill ||
            ...
            return (
              ...
              >
                {formatter &&
                (item as { value?: unknown; name?: string }).value !==
                  undefined &&
                (item as { name?: string }).name ? (
                  formatter(
                    (item as { value: ValueType }).value,
                    (item as { name: NameType }).name,
                    item as never,
                    index,
                    (item as { payload?: unknown }).payload as never,
                  )
                ) : (
                  <>
                    {itemConfig?.icon ? (
...
// Re-export specific Recharts components if needed, or consumers can import from recharts
